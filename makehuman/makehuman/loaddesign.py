...
custom = {
    ...
    "eyes": "Name of a file in data/eyes/*/ without the extension",
    "teeth": "Name of a file in data/teeth/*/ without the extension",
    "hair": "Name of a file in data/hair/*/ without the extension",
    "eyebrows": "Name of a file in data/eyebrows/*/ without the extension",
    "eyelashes": "Name of a file in data/eyelashes/*/ without the extension",
    "tongue": "Name of a file in data/tongue/*/ without the extension",
...
def savevalues(filename):
    with open(filename, 'w') as fh:
        ...
        values['skin'] = barename(human.getMaterial().filename)
        proxies = human.getProxies(includeHumanProxy=False)
        clothes = []
        for p in proxies:
            if '/eyes/' in p.file:
                values['eyes'] = barename(p.file)
            elif '/teeth/' in p.file:
                values['teeth'] = barename(p.file)
            elif '/hair/' in p.file:
                values['hair'] = barename(p.file)
            elif '/eyebrows/' in p.file:
                values['eyebrows'] = barename(p.file)
            elif '/eyelashes/' in p.file:
                values['eyelashes'] = barename(p.file)
            elif '/tongue/' in p.file:
                values['tongue'] = barename(p.file)
            elif '/clothes/' in p.file:
                clothes.append(barename(p.file))
        if clothes:
